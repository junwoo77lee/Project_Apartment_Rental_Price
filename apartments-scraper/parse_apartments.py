...
def create_csv(search_urls, map_info, fname, pscores):
    ...
    try:
        ...
        # write_parsed_to_csv)
        header = ['Option Name', 'Contact', 'Address', 'Size_1Bedroom', 'Size_2Bedrooms',
                  'Rent', 'Rent_1Bedroom', 'Rent_2Bedrooms',
...
def write_parsed_to_csv(page_url, map_info, writer, pscores):
    ...
    for item in soup.find_all('article', class_='placard'):
        ...
        row = [fields['name'], contact,
               fields['address'], fields['size_1BR'], fields['size_2BR'],
               rent, fields['1BR'], fields['2BRs'], fields['monthFees'], fields['onceFees'],
...
def parse_apartment_information(url, map_info):
    ...
    # save html to local whatever.html file
    ...
    # get rental price for each bedroom types: 1 or 2 bedrooms
    get_rental_price_by_bedroom_type(soup, fields)
...
def get_property_size(soup, fields):
    ...
    # 1 and 2 bedrooms
    for bedroom_type in ['1', '2']:
        obj = soup.find('tr', {'data-beds': bedroom_type})
        if obj is not None:
            data = obj.find('td', class_='sqft').getText()
            data = prettify_text(data)
            field_name = 'size_' + bedroom_type + 'BR'
            fields[field_name] = data

def get_rental_price_by_bedroom_type(soup, fields):
    fields['1BR'] = ''
    fields['2BRs'] = ''

    if soup is None: return

    obj = soup.find_all(name='span', attrs={'class' : 'rentRollup'})
    if obj is not None:
        for _ in range(len(obj)):  # iterating up to total amount of bedroom types in an apartment
            bedroom_type = obj[_].findNext('span').contents[0] # ex. 'Studio', '1 Bedroom', etc.
            if bedroom_type in ['1 Bedroom', '2 Bedrooms']:                
                rental_price_range = re.findall('[\d,]{3,}', obj[_].text)  #.split("$")[-1].split("\r")[0].replace(",", "").split("–")
                rental_price_range = '-'.join(rental_price_range).replace(",", "")
                if bedroom_type == '1 Bedroom':
                    fields['1BR'] = rental_price_range
                else: # 2 Bedrooms
                    fields['2BRs'] = rental_price_range
